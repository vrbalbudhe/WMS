    ...
    return (
      <div className="p-4">
        ...
        <div className="mb-4">
          ...
          <div id="reader" className="w-full max-w-md mx-auto border rounded"></div>
        </div>
  ...
  if (loading || isSaving) {
    return (
      <div className="p-4 text-center">
        {isSaving ? "Saving scan data to database..." : "Loading product information..."}
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-4">
        ...
        <div className="mb-4">
          <span className="font-semibold">Warehouse ID:</span> {product.warehouseId || warehouseId}
        </div>
        
        {(product.warehouseOfficer || warehouseOfficer) && (
          <div className="mb-4">
            <span className="font-semibold">Warehouse Officer:</span> {product.warehouseOfficer || warehouseOfficer}
          </div>
        )}
        
        <div className="mb-4">
          <span className="font-semibold">Status:</span> {product.status || status || 'qr'}
        </div>
        
        {product.scanTimestamp && (
          <div className="mb-4">
            <span className="font-semibold">Scan Time:</span> {new Date(product.scanTimestamp).toLocaleString()}
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div>
              ...
              {product.inStock !== undefined && (
                <p><span className="font-medium">In Stock:</span> {product.inStock ? "Yes" : "No"}</p>
              )}
            </div>
          </div>
        </div>
